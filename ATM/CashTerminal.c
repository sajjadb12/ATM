...
#define WM_KARD                   0x000F
#define WM_MABLAGH                0x000E
//#define GUI_ID_EDIT1              0x10000
...
#define TEXT_ID_CARD_TO_CARD        19
#define TEXT_ID_SHOMARE_HESAB       20
#define TEXT_ID_MABLAGH             21
...
static const char * _aLang[][2] = {
  ...
  { "Ramze Khod Ra Vared Konid:",
    "Please enter your PIN:"
  },
  { "Karte Khod Ra Vared Konid:",
  ...
  { "Lotfan Entekhab lonid...",
    "Please select..."
  },
  { "Mabaleghe Digar",
    "Different amount"
  },
  { "Lotfan Montezer Bemanid.\nDastgah Dar Hal Anjam Tarakomnesh.",
    "Please wait until your transaction\nis being processed."
  },
  { "Lotfan Karte Khod Ra Bardarid",
    "Please you take your card"
  },
  { "Tarakonesh Ba Moafaghiat Anjam Shod.\nLotfan Pool Va Resid Khod Ra Bardarid.",
  ...
  { "Mojoodi Shoma: 17.000 Rial",
    "Your balance amount is: 17.000 Rial"
  },
  { "Mablagh Ra Entekhab Konid",
...
static void _cbThanks(WM_MESSAGE* pMsg) {
  ...
  case WM_PAINT:
    _PaintFrame();
    GUI_DispStringHCenterAt(_GetLang(TEXT_ID_VIELEN_DANK), FRAME_WIDTH >> 1, 80);
...
static void _cbCardToCard(WM_MESSAGE* pMsg);
static void _cbCardToCard(WM_MESSAGE* pMsg) {
  char acBuffer[20] = {0};
  char * pBuffer;
  WM_HWIN hItem, hWin = pMsg->hWin;
  ...
  case WM_CREATE:
    WM_SetFocus(hWin);
    _CreateNumPad(hWin, FRAME_WIDTH >> 1, 280);
    hItem = EDIT_CreateEx((FRAME_WIDTH >> 1) - 200, 90, 400, 37, hWin, WM_CF_SHOW, 0, GUI_ID_EDIT0, 4);
	//hItem = EDIT_CreateEx((FRAME_WIDTH >> 1) - 50, 160, 100, 37, hWin, WM_CF_SHOW, 0, GUI_ID_EDIT1, 4);
    EDIT_SetFont(hItem, &GUI_FontAA2_32);
    EDIT_SetTextAlign(hItem, GUI_TA_CENTER);
    WIDGET_SetEffect(hItem, DEFAULT_WIDGET_EFFECT);
    WM_DisableWindow(hItem);
  ...
  case WM_KARD:
    _PaintFrame();
    GUI_DispStringHCenterAt(_GetLang(TEXT_ID_SHOMARE_HESAB), FRAME_WIDTH >> 1, 15);/**/
    break;
  //case WM_MABLAGH:
  //  _PaintFrame();
  //  GUI_DispStringHCenterAt(_GetLang(TEXT_ID_MABLAGH), FRAME_WIDTH >> 1, 14);/**/
  //  break;
  case MSG_PIN_ERROR:
    if (!_PIN_Digits) {
      break;
...
static void _cbMablagh(WM_MESSAGE* pMsg);
static void _cbMablagh(WM_MESSAGE* pMsg) {
  char acBuffer[20] = {0};
  char * pBuffer;
  WM_HWIN hItem, hWin = pMsg->hWin;
  ...
  case WM_CREATE:
    WM_SetFocus(hWin);
    _CreateNumPad(hWin, FRAME_WIDTH >> 1, 280);
    hItem = EDIT_CreateEx((FRAME_WIDTH >> 1) - 200, 90, 400, 37, hWin, WM_CF_SHOW, 0, GUI_ID_EDIT0, 16);
	//hItem = EDIT_CreateEx((FRAME_WIDTH >> 1) - 50, 160, 100, 37, hWin, WM_CF_SHOW, 0, GUI_ID_EDIT1, 4);
    EDIT_SetFont(hItem, &GUI_FontAA2_32);
    EDIT_SetTextAlign(hItem, GUI_TA_CENTER);
    WIDGET_SetEffect(hItem, DEFAULT_WIDGET_EFFECT);
  ...
  case WM_KARD:
    _PaintFrame();
    GUI_DispStringHCenterAt(_GetLang(TEXT_ID_MABLAGH), FRAME_WIDTH >> 1, 15);/**/
    break;
  //case WM_MABLAGH:
  //  _PaintFrame();
  //  GUI_DispStringHCenterAt(_GetLang(TEXT_ID_MABLAGH), FRAME_WIDTH >> 1, 14);/**/
...
static void _cbSelectMoney(WM_MESSAGE* pMsg) {
  ...
  case WM_CREATE:
    WM_SetFocus(hWin);
    _CreateButton(hWin,  "500,000 R",      GUI_ID_BUTTON0,  25,  84, 270,  60, 0);
    _CreateButton(hWin, "1,000,000 R",     GUI_ID_BUTTON1,  25, 162, 270,  60, 0);
    _CreateButton(hWin, "1,500,000 R",     GUI_ID_BUTTON2,  25, 240, 270,  60, 0);
    _CreateButton(hWin, "2,000,000 R",     GUI_ID_BUTTON3,  25, 318, 270,  60, 0);
    _CreateButton(hWin, "2,500,000 R",     GUI_ID_BUTTON4, 320,  84, 270,  60, 0);
    _CreateButton(hWin, "3,000,000 R",     GUI_ID_BUTTON5, 320, 162, 270,  60, 0);
    _CreateButton(hWin, "3,500,000 R",     GUI_ID_BUTTON6, 320, 240, 270,  60, 0);
...
static void _cbMainMenu(WM_MESSAGE* pMsg) {        
  ...
  case WM_CREATE:
    WM_SetFocus(hWin);
    _CreateButton(hWin, "", GUI_ID_BUTTON0, (FRAME_WIDTH >> 1) - 175, 140, 350,  60, TEXT_ID_GELD_ABHEBEN);
    _CreateButton(hWin, "", GUI_ID_BUTTON1, (FRAME_WIDTH >> 1) - 175, 220, 350,  60, TEXT_ID_KONTOSTAND_ANSEHEN);
	_CreateButton(hWin, "", GUI_ID_BUTTON2, (FRAME_WIDTH >> 1) - 175, 300, 350,  60, TEXT_ID_CARD_TO_CARD);
    break;
  case WM_PAINT:
    _PaintFrame();
...
static void _cbLanguage(WM_MESSAGE* pMsg) {
  ...
  case WM_CREATE:
    WM_SetFocus(hWin);
    _CreateButton(hWin, "Farsi", GUI_ID_BUTTON0, (FRAME_WIDTH >> 1) - 175, 162, 350,  60, 0);
    _CreateButton(hWin, "English", GUI_ID_BUTTON1, (FRAME_WIDTH >> 1) - 175, 240, 350,  60, 0);
    break;
  case WM_PAINT:
    _PaintFrame();
    GUI_DispStringHCenterAt("Zabane Khod Ra Entekhab Konid", FRAME_WIDTH >> 1, 15);
    break;
  case WM_NOTIFY_PARENT:
    if (pMsg->Data.v == WM_NOTIFICATION_RELEASED) {
      int Id = WM_GetId(pMsg->hWinSrc);
